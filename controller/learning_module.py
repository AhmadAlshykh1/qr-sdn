...
def learning_module(pipe, ):
    ...
    logging.info('Starting learning module...') 
        ...
        if len(elements) > 0:
            ...
            if len(elements['currentCombination']) > 0:
                ...
                if reset_load_flag:
                    ...
                    logging.info("change in load level. new load level: %s", load_level)
                ...
                if reset_iteration_flag:
                    ...
                    logging.info("xxxxxxxx RESETTING Q LoadLevel: %s Iteration: %s xxxxxxxxxxxxx", load_level, iterations_level)
                    clearing_save_file(log_path, load_level, 'reward_controller', split_up_load_levels,
                                       iteration_split_up_flag, iterations_level)
                    clearing_save_file(log_path, load_level, 'average_latency', split_up_load_levels,
                                       iteration_split_up_flag, iterations_level)
                ...
                if load_level > 0:
                    ...
                    if len(temp_flows) < 1:
                        ...
                        logging.info("Average lat: %s reward: %s", average_latency_list, rewards_list)
                        ...
                        if len(rewards_list) >= measurements_for_reward:
                            ...
                            if learning_mode.value != QMode.SHORTEST_PATH.value:
                                ...
                                logging.info("Action: %s Next State: %s PrevState: %s PrevReward: %s", previous_action, current_state, previous_state, np.mean(rewards_list))
                            # log output
                            if (general_iterator % 100) < 1:
                                    logging.info("-------number of batch: %s epsilon: %s", general_iterator, epsilon)
                ...
                if int((time.time() - starting_time) / 60) > duration_per_minutes:
                    save_q(Q)
                    logging.info("Exited after %s steps (last load level)",general_iterator)
                    break
...
def update_q_ql(current_state, next_state, alpha, gamma, Q, reward, action):
    ...
    try:
        ...
        Q[state_now_str][action_str][0] = Q[state_now_str][action_str][0] + 1
        logging.info(f"Updating Q-table, state_now_str: {state_now_str}, next_state_str: {next_state_str}, action_str: {action_str}, reward: {reward} New Q Value: {Q[state_now_str][action_str][1]}") # تسجيل إضافي
    except KeyError:
        print("Q: {}".format(Q))
        print("StateNowStr: {}".format(state_now_str))
    return Q
